...
public class Login extends AppCompatActivity
{
    ...
    public void register(View view) {
        ...
        while(c.moveToNext())
        {
            if(c.getString(3).equals(uname))
            {
                isRight=false;
                Toast.makeText(this, "Username Already Exists", Toast.LENGTH_SHORT).show();
                return;
            }
            if(c.getString(2).equals(pho))
            {
                isRight=false;
                Toast.makeText(this, "Phone-number Already Exists, Try New One!!", Toast.LENGTH_SHORT).show();
                return;
            }
        ...
        if(nm.length()<5 || uname.length()<5){
            Toast.makeText(this, "Name and Username should 5 chars long!", Toast.LENGTH_SHORT).show();
            isRight=false;
            return;
        }
        if(pho.length()!=10){
